...
class DataManager:
    ...
    def __init__(self):
        ...
        # 内存数据
        self.user_mapping: Dict[int, int] = {}      # 消息ID -> 用户ID
        self.whitelist: Set[int] = set()            # 白名单
        self.blacklist: Set[int] = set()            # 黑名单
        self.pending_verify: Dict[int, dict] = {}   # 待验证
        ...
        # 优化：用户消息索引缓存
        self._user_message_index: Dict[int, List[int]] = {}  # {user_id: [msg_ids]}
    ...
    def _rebuild_user_index(self):
        """重建用户消息索引"""
        self._user_message_index.clear()
        for msg_id, user_id in self.user_mapping.items():
            if user_id not in self._user_message_index:
                self._user_message_index[user_id] = []
            self._user_message_index[user_id].append(msg_id)
        logger.info(f"用户索引重建完成: {len(self._user_message_index)} 个用户")
    ...
    def _load_json(self, filepath: str, attr_name: str, key_type=None, as_set=False):
        """通用JSON加载"""
        try:
    ...
    def mark_dirty(self, data_type: str):
        """标记数据已修改"""
        if data_type in self._dirty_flags:
            self._dirty_flags[data_type] = True
    
    async def auto_save_loop(self):
        """定时自动保存循环"""
        logger.info(f"自动保存循环已启动 (间隔: {AUTO_SAVE_INTERVAL}秒)")
        while True:
    ...
    async def save_if_dirty(self):
        """保存所有脏数据"""
        saved = []
        if self._dirty_flags['mapping']:
            self.save_mapping()
            self._dirty_flags['mapping'] = False
            saved.append('mapping')
        if self._dirty_flags['whitelist']:
            self.save_whitelist()
            self._dirty_flags['whitelist'] = False
            saved.append('whitelist')
        if self._dirty_flags['blacklist']:
            self.save_blacklist()
            self._dirty_flags['blacklist'] = False
            saved.append('blacklist')
        if self._dirty_flags['pending']:
            self.save_pending()
            self._dirty_flags['pending'] = False
            saved.append('pending')
        if self._dirty_flags['stats']:
            self.save_stats()
            self._dirty_flags['stats'] = False
            saved.append('stats')
        
        if saved:
            logger.debug(f"自动保存完成: {', '.join(saved)}")
    ...
    def update_user_index(self, msg_id: int, user_id: int):
        """更新用户消息索引"""
        if user_id not in self._user_message_index:
            self._user_message_index[user_id] = []
        self._user_message_index[user_id].append(msg_id)
    ...
    def get_user_messages(self, user_id: int) -> List[int]:
        """获取用户的所有消息ID"""
        return self._user_message_index.get(user_id, [])
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id in dm.whitelist:
        try:
            ...
            failed += 1
            failed_users.append((user_id, str(e)[:50]))
            logger.warning(f"广播失败 {user_id}: {e}")
    ...
    if failed_users and len(failed_users) <= 5:
        ...
        for uid, err in failed_users:
            result_text += f"• <code>{uid}</code>: {err}\n"
...
async def forward_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        dm.update_user_index(forwarded.message_id, user.id)
        dm.mark_dirty('mapping')
        ...
        logger.info(f"转发消息: {user.id} -> 主人 (耗时: {elapsed:.2f}s)")
...
async def post_init(application: Application):
    ...
    # 启动自动保存循环
    asyncio.create_task(dm.auto_save_loop())
    ...
    try:
        await application.bot.send_message(
            ...
            text=(
                ...
                "✨ 已启用性能优化\n"
                f"⏱️ 自动保存间隔: {AUTO_SAVE_INTERVAL}秒\n\n"
                "输入 /help 查看命令"
            ),
            parse_mode=ParseMode.HTML
        )
